  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setInfrastructureMetrics({
        ...
        avgCpuUsage: Math.round(mockServers.reduce((acc, s) => acc + s.cpu, 0) / mockServers.length),
        avgMemoryUsage: Math.round(mockServers.reduce((acc, s) => acc + s.memory, 0) / mockServers.length),
      });
      setLoading(false);
    }, 1000);
  }, []);
  ...
  return (
    <div className="p-6 max-w-7xl ">
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          <h1 className="text-3xl font-bold text-neutral-900 dark:text-white">Hosting & Infrastructure</h1>
          <p className="text-neutral-600 dark:text-neutral-400 mt-2">
            Monitor and manage server infrastructure and deployments
          </p>
        </div>
        <div className="flex gap-3">
          <button className="px-4 py-2 text-sm font-medium text-neutral-700 dark:text-neutral-300 bg-white dark:bg-secondary-800 border border-neutral-300 dark:border-secondary-600 rounded-lg hover:bg-neutral-50 dark:hover:bg-secondary-700 transition-colors">
            Refresh Status
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white dark:bg-secondary-800 rounded-xl p-6 shadow-sm border border-neutral-200 dark:border-secondary-700">
          <h4 className="text-sm font-medium text-neutral-600 dark:text-neutral-400 mb-2">Total Servers</h4>
          <div className="text-2xl font-bold text-neutral-900 dark:text-white">{infrastructureMetrics.totalServers}</div>
        </div>
        <div className="bg-white dark:bg-secondary-800 rounded-xl p-6 shadow-sm border border-neutral-200 dark:border-secondary-700">
        ...
        <div className="bg-white dark:bg-secondary-800 rounded-xl p-6 shadow-sm border border-neutral-200 dark:border-secondary-700">
          ...
          <div className="text-2xl font-bold text-neutral-900 dark:text-white">{infrastructureMetrics.avgCpuUsage}%</div>
        </div>
        <div className="bg-white dark:bg-secondary-800 rounded-xl p-6 shadow-sm border border-neutral-200 dark:border-secondary-700">
          <h4 className="text-sm font-medium text-neutral-600 dark:text-neutral-400 mb-2">Avg Memory Usage</h4>
          <div className="text-2xl font-bold text-neutral-900 dark:text-white">{infrastructureMetrics.avgMemoryUsage}%</div>
        </div>
      </div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-neutral-600 dark:text-neutral-400">{server.ip} • {server.location}</p>
                      </div>
                    </div>
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(server.status)}`}>
                    ...
                    {[
                      ...
                      <div key={resource.label}>
                        <div className="flex justify-between text-sm mb-1">
                          <span className="text-neutral-600 dark:text-neutral-400">{resource.label}</span>
                          <span className="font-medium text-neutral-900 dark:text-white">{resource.value}%</span>
                        </div>
                        <div className="w-full bg-neutral-200 dark:bg-secondary-700 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full ${getResourceBarColor(resource.value)}`}
          ...
          {deployments.map((deployment) => {
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h4 className="font-semibold text-neutral-900 dark:text-white">{deployment.name}</h4>
                    <p className="text-sm text-neutral-600 dark:text-neutral-400">
                      {deployment.environment} • {deployment.branch} • {deployment.commit}
                    </p>
                  </div>
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(deployment.status)}`}>
                    <StatusIcon className="h-3 w-3" />
                    {deployment.status}
                  </span>
                </div>
                <div className="text-sm text-neutral-600 dark:text-neutral-400">
                  ...
                  {deployment.endTime && (
                    <p><strong>Completed:</strong> {new Date(deployment.endTime).toLocaleString()}</p>
                  )}
