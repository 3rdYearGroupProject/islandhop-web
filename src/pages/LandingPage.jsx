    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="w-full flex justify-center">
          <div className="flex flex-nowrap gap-3 px-2 justify-center">
            ...
            {[
              ...
              return (
                <div key={name} className="flex flex-col items-center justify-end min-w-[6rem] max-w-[6rem] w-24 h-32 mx-1 cursor-pointer group" style={{ marginTop: '-1.5rem' }}>
      ...
      <section className="w-full py-8">
        <div className="content-container">
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
            {[
              { name: 'Sigiriya Rock Fortress', image: sigiriyaImg, description: 'Ancient rock fortress and palace ruins' },
              { name: 'Temple of the Tooth (Kandy)', image: templesImg, description: 'Sacred Buddhist temple housing Buddha\'s tooth relic' },
              { name: 'Galle Fort', image: galleFortImg, description: 'Historic Dutch colonial fortification' },
              { name: 'Nine Arches Bridge (Ella)', image: hikingImg, description: 'Iconic railway bridge in hill country' },
              { name: 'Yala National Park', image: wildlifeImg, description: 'Premier wildlife safari destination' },
              { name: 'Dambulla Cave Temple', image: templesImg, description: 'Ancient cave temple complex with Buddha statues' }
            ].map((place, i) => (
              <div key={place.name} className="flex-shrink-0 w-64 md:w-72">
                <DestinationCard
                  destination={place}
                  imageUrl={place.image}
                  onClick={() => navigate(`/place/${place.name.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '')}`, { state: place })}
                  className="h-48 md:h-56"
                />
              </div>
            ))}
