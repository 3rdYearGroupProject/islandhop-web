...
const DriverChat = () => {
  ...
  const getDriverName = () => {
    if (currentUser?.displayName) {
      ...
      return `Driver - ${currentUser.displayName}`;
    }
    if (userData?.firstName) {
      console.log('Using stored user data name:', userData.firstName, userData.lastName);
      return `Driver - ${userData.firstName} ${userData.lastName || ''}`.trim();
    }
    console.log('Using fallback UID name');
    return `Driver - ${currentUserUID?.substring(0, 8) || 'User'}`;
  };
  
  const currentUserName = getDriverName();
