...
const DriverEarnings = () => {
  ...
  const fetchEarningsData = async () => {
    try {
      ...
      const [earningsRes, transactionsRes] = await Promise.all([
        axios.get(
          `http://localhost:5001/api/drivers/${driverEmail}/earnings?period=${timeFilter}`
        ),
        axios.get(
          `http://localhost:5001/api/drivers/${driverEmail}/transactions?limit=10`
        ),
      ]);

      // Transform earnings data to match component expectations
      const rawEarningsData = earningsRes.data.success
      ...
      const mockDailyData = [
        ...
        {
          day: "Tue",
          earnings: (rawEarningsData.weeklyEarnings || 1240) * 0.15,
        },
        {
          day: "Wed",
          earnings: (rawEarningsData.weeklyEarnings || 1240) * 0.12,
        },
        {
          day: "Thu",
          earnings: (rawEarningsData.weeklyEarnings || 1240) * 0.18,
        },
        {
          day: "Fri",
          earnings: (rawEarningsData.weeklyEarnings || 1240) * 0.25,
        },
        {
          day: "Sat",
          earnings: (rawEarningsData.weeklyEarnings || 1240) * 0.2,
        },
        {
          day: "Sun",
          earnings: (rawEarningsData.weeklyEarnings || 1240) * 0.1,
        },
      ];
      ...
      const mockWeeklyData = [
        {
          week: "Week 1",
          earnings: (rawEarningsData.monthlyEarnings || 4820) * 0.25,
        },
        {
          week: "Week 2",
          earnings: (rawEarningsData.monthlyEarnings || 4820) * 0.28,
        },
        {
          week: "Week 3",
          earnings: (rawEarningsData.monthlyEarnings || 4820) * 0.22,
        },
        {
          week: "Week 4",
          earnings: (rawEarningsData.monthlyEarnings || 4820) * 0.25,
  ...
  const fetchBankDetails = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5001/api/drivers/${driverEmail}/bank-details`
      );
  ...
  const saveBankDetails = async () => {
    try {
      ...
      setBankError(
        "Bank details saving is not yet implemented in the backend."
      );
  ...
  const exportEarningsToPDF = () => {
    try {
      ...
      doc.text("IslandHop Driver Earnings Report", pageWidth / 2, 20, {
        align: "center",
      });

      doc.setFontSize(10);
      doc.setFont(undefined, "normal");
      doc.text(`Driver: ${driverEmail || "N/A"}`, pageWidth / 2, 30, {
        align: "center",
      });
      ...
      if (timeFilter === "week") {
        summaryData.push([
          "Daily Average",
          formatCurrency(currentData.total / 7),
        ]);
      } else if (timeFilter === "month") {
        summaryData.push([
          "Weekly Average",
          formatCurrency(currentData.total / 4),
        ]);
      }
      ...
      doc.setTextColor(59, 130, 246);
      doc.text(
        `${timeFilter === "week" ? "Daily" : "Weekly"} Breakdown`,
        14,
        yPos
      );
      doc.setTextColor(0, 0, 0);
      ...
      const breakdownData = (
        timeFilter === "week" ? currentData.daily : currentData.weekly
      ).map((item) => [
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text("IslandHop - Confidential Driver Report", 14, pageHeight - 10);
        doc.text(
      ...
      // Save the PDF
      const fileName = `Driver_Earnings_${timeFilter}_${
  ...
  const averagePerTrip =
    currentData.trips > 0 ? currentData.total / currentData.trips : 0;
              ...
              <div>
                ...
                {isEditingBank ? (
                  ...
                  >
                    ...
                    <option value="National Bank of Sri Lanka">
